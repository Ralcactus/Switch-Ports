...
if (!global.panic)
{
	...
	if (!is_undefined(target_room_music) && is_struct(target_room_music))
	{
		...
		if (is_undefined(previous_room_music) || previous_room_music.eventName != target_room_music.eventName)
		{
			...
			global.CurrentMusic = target_room_music.musicInst;
...
if (global.RoomIsSecret)
{
	...
	if (!is_undefined(global.RoomMusic) && !is_undefined(global.RoomMusic.secretMusicInst))
	{
		...
		var music_timeline_percent = fmod_studio_event_instance_get_timeline_position(global.CurrentMusic) / fmod_getEventLength(global.RoomMusic.eventName)
		...
		fmod_studio_event_instance_set_paused(global.CurrentMusic, true)
