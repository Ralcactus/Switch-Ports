        ...
        if (obj_mainmenu_controller.opening_timer > 0.95 && obj_mainmenu_controller.active_menu == self)
        {
            switch (anim)
            {
                case 0:
                    ...
                    {
                        if (t > 1 && (keyboard_check_pressed(vk_space) || global.gamepad_select))
                        {
                            space_hold = true;
                        }
                        t++;
...
with (menu_options)
{
    ...
    tick_before = function()
    {
        ...
        if (menu.input_allowed && focused && menu.screen == 0 && (keyboard_check_pressed(vk_escape) || gamepad_button_check_pressed(global.P1_input, gp_start)))
        {
            menu.on_exit();
