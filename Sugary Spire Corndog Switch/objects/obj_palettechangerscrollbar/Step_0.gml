...
if (mouse_check_button_pressed(mb_left) && !input_text && !obj_palettechanger.stop)
{
    if (point_in_circle(_mouse_x, _mouse_y, button_x, draw_y, button_radius))
        selected = true;
}
else if (!mouse_check_button(mb_left) && !input_text)
{
    selected = false;
}
...
if (mouse_check_button_pressed(mb_left) && !obj_palettechanger.stop && !selected && !input_text && !obj_palettechanger.stop)
{
...
if ((player.key_jump2 || keyboard_check_pressed(vk_enter) || player.key_slap2) && !selected && input_text)
{
    input_text = false;
...
if (input_text)
{
    ...
    var _keypressed = string_length(is_number) ? string_length(is_number) : is_backspace;
