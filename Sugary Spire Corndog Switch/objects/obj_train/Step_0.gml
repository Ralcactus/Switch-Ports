switch (state)
{
    case UnknownEnum.Value_1:
        ...
        if (grounded)
        {
            if (distance_to_point(obj_parent_player.x, y) > 700 && rubberband < 3 && ((obj_parent_player.x <= x && image_xscale <= -1) || (obj_parent_player.x >= x && image_xscale >= 1)))
            {
        ...
        if (place_meeting(x + hsp, y, obj_chocofrog) || place_meeting(x + sign(image_xscale), y, obj_chocofrog))
        {
            with (instance_place(x + hsp, y, obj_chocofrog))
                instance_destroy();
            
            with (instance_place(x + sign(image_xscale), y, obj_chocofrog))
                instance_destroy();
        }
