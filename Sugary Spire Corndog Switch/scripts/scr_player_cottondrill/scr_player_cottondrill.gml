function state_player_cottondrill()
{
    ...
    if (sprite_index == spr_player_PZ_cotton_drill)
    {
    ...
    else
    {
        if (vsp > 0.5 && !grounded)
        {
            ...
            vsp = 0;
            hsp = 0;
            sprite_index = spr_player_PZ_cotton_doubleJump_fall;
    ...
    if (grounded && vsp > 0.5 && !place_meeting(x, y + 1, obj_destructibles) && !place_meeting(x, y + 1, obj_chocofrog))
    {
        ...
        {
            state = UnknownEnum.Value_87;
            sprite_index = spr_player_PZ_cotton_land;
            vsp = 0;
            movespeed = 0;
            image_index = 0;
        }
    ...
    if (inputBufferSlap > 0 && sprite_index != spr_player_PZ_cotton_attack && groundedCot && !place_meeting(x, y, obj_cottonsolid))
    {
        inputBufferSlap = 0;
        state = UnknownEnum.Value_87;
        flash = true;
