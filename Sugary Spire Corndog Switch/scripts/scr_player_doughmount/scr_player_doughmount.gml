function state_player_doughmount()
{
    ...
    if (move != 0 && inputBuffer <= 0)
    {
        ...
        if (movespeed != 0 && place_meeting_collision(x + sign(xscale), y, UnknownEnum.Value_1) && !place_meeting(x + sign(xscale), y, obj_destructibles) && !place_meeting(x + sign(xscale), y, obj_chocofrog))
        {
            inputBuffer = 5;
    ...
    if (sprite_index == spr_player_PZ_dogMount_skid && animation_end())
        sprite_index = spr_player_PZ_dogMount;
    
    if (sprite_index == spr_player_PZ_dogMount_inflate_jump && animation_end())
...
function state_player_doughmountspin()
{
    ...
    if (animation_end() && sprite_index == spr_player_PZ_dogMount_spin)
    {
        image_index = 0;
        sprite_index = spr_player_PZ_dogMount;
        state = UnknownEnum.Value_78;
        movespeed *= xscale;
    }
...
function state_player_doughmountballoon()
{
    ...
    if (sprite_index == spr_player_PZ_dogMount_inflate && animation_end())
