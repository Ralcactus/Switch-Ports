function scr_enemy_hit()
{
    ...
    if ((vsp > 0 && place_meeting_collision(x, y + 1) && !place_meeting(x, y + 1, obj_destructibles)) || (hsp != 0 && place_meeting_solid(x + sign(hitHsp), y) && !place_meeting(x + sign(hitHsp), y, obj_destructibles)))
    {
        vsp = -5;
        weakThrowHit = false;
    ...
    if ((vsp < 0 || hitVsp != 0) && place_meeting_solid(x, y - 1) && !place_meeting(x, y + hitVsp, obj_destructibles) && (!weakThrowHit || shoulderbashed <= 0))
    {
        vsp = 5;
        weakThrowHit = false;
        impact = true;
    ...
    if (impact)
    {
        ...
        if (old_weakThrowHit != weakThrowHit)
        {
            flash = true;
            weakThrowHit = false;
            throwAntiGrav = false;
            baddieStunTimer = max(baddieStunTimer, 100);
