...
var _inst = instance_create_layer(-64, -512, "Over", obj_sketchcollect_solid);
_inst.image_yscale = 22;
_inst.ID = ID;
_inst.controller = controller;
_inst = instance_create_layer(1590, -512, "Over", obj_sketchcollect_solid);
_inst.image_yscale = 22;
_inst.ID = ID;
_inst.controller = controller;
var _choice = choose("A", "B");
var __data = struct_get(global.microgames_data, string("sketch_collect{0}", _choice));
var _data = json_load(__data, false);

for (var i = 0; i < array_length(_data.instances); i++)
{
    _inst = _data.instances[i];
    ...
    _new.ID = controller.ID;
    _new.player = controller.player;
    _new.visible = false;
}
