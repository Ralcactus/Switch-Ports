/*
the MASTERCLIP shader is written by certusquark,
so if u plan on yoinking this code... credit them!
dont steal work without crediting u donut...
- vinny
*/
...
vec4 mask_clip(vec4 color)
{
	...
	// Quark: are we inside the clip region?
	if	(
		...
		)
	{
		...
		game_out_color = vec4(0.0, 0.0, 0.0, 0.0); // Quark: Outside so it will be invisible
...
vec4 rect_clip(vec4 color)
{
	...
	game_out_color.a = Vec4ContainsVec2(clip_bounds, v_vPosition) ? game_out_color.a : 0.0;
