...
function draw_quick_score_list(arg0, arg1, arg2, arg3 = {})
{
    ...
    if (struct_exists(arg3, "width"))
        _width = arg3.width;
    
    var _v_separation = 35;
    
    if (struct_exists(arg3, "v_separation"))
        _v_separation = arg3.v_separation;
    
    var _h_separation = 0;
    
    if (struct_exists(arg3, "h_separation"))
        _h_separation = arg3.h_separation;
    
    var scores_count = 3;
    
    if (struct_exists(arg3, "scores_count"))
        scores_count = arg3.scores_count;
    
    scores_count = clamp(scores_count, 0, 3);
    
    if (_scores != undefined)
    {
        for (var i = 0; i < scores_count; i++)
        {
            var drawx = arg0 + (_h_separation * ((i - (scores_count / 2)) + 0.5));
            var drawy = arg1 + (_v_separation * ((i - (scores_count / 2)) + 0.5));
            var _score = 0;
